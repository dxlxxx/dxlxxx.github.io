...
function Light() {
  ...
  return (
    <div>
      <button style={{background: theme.background, color: theme.foreground}}>没有Provide使用createContext的默认值</button>
...
function Dark() {
  ...
  return (
    <div>
      <button style={{background: theme.background, color: theme.foreground}}>有Provider使用Provider的value值</button>
    </div>
  )
}
